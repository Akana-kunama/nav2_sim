...
void RobotAwait::send_navigation_goal(const std::string& position_name)
{
    ...
    nlohmann::json navigation_goals;
    input_file >> navigation_goals;

    // 检查目标位置是否存在
    if (navigation_goals.find(position_name) == navigation_goals.end())
    {
    ...
    // 获取目标位置的坐标
    auto position_data = navigation_goals[position_name];
    double x = position_data["x"];
    double y = position_data["y"];
    double orientation_w = position_data["orientation_w"];
